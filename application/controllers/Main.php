...
class Main extends CI_Controller
{
	...
	public function info_save()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$data = array(
				'title' => $this->input->post('title'), 'deskripsi' => base64_encode($this->input->post('deskripsi')), 'ket' => base64_encode($this->input->post('ket')), 'footer_invoice' => base64_encode($this->input->post('footer')), 'email' => $this->input->post('email'), 'perusahaan' => $this->input->post('perusahaan'), 'phone' => $this->input->post('phone'), 'fb' => $this->input->post('fb'), 'ig' => $this->input->post('ig'), 'api_key' => $token_api, 'logo' => $nama_logo, 'logo_bw' => $nama_logo_bw, 'stamp_l' => $lunas, 'stamp_b' => $blunas, 'warna_lunas' => $this->input->post('warna_lunas'), 'warna_blunas' => $this->input->post('warna_blunas'), 'tema' => $this->input->post('tema'), 'keywords' => $this->input->post('keywords'), 'kode_qris' => $qris, 'dev_tools' => $this->input->post('devtools', TRUE), 'favicon' => $nama_icon
			);
			$where = array('id' => 1);
