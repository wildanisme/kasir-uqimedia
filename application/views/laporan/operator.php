	...
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
		...
		<div class="breadcrumb">
			<div class="input-group input-group-sm">
				...
				<select name="produk" id="produk" class="form-control form-control-sm custom-select" onchange="search_Operator()">
					<option value="0">SEMUA</option>
					<?php  
						foreach ($produk AS $values){
							echo '<option value="'.$values['id'].'">'.strtoupper($values['title']).'</option>';
						}
					?>
				</select>
			</div>
			...
			<div class="card">
				...
				<div class="card-header pb-2">
					...
					<div class="input-group input-group-sm">
						...
						<input type="text" onchange="search_Operator()"  class="form-control form-control-sm w-100px" name="invoice_op" id="invoice_op" placeholder="No.Order">
...
<script>
	...
	function search_Operator(page_num){
		...
		$.ajax({
			type: 'POST',
			url: urlnya,
			data:{page:page_num,user:user,dari:dari,sampai:sampai,invoice_op:invoice_op,limits:limits,status:status,produk:produk},
			beforeSend: function(){
				$('body').loading();
			},
