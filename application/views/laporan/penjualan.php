					...
					<div class="nav-tabs-navigation">
						<div class="nav-tabs-wrapper">
							<ul class="nav nav-tabs" data-tabs="tabs">
								<li class="nav-item">
									<a class="nav-link active" href="#produk" data-toggle="tab">Semua Produk</a>
								</li>
								<li class="nav-item">
									<a class="nav-link" href="#kategori" data-toggle="tab">Semua Kategori</a>
								</li>
								<li class="nav-item">
									<a class="nav-link" href="#grup" data-toggle="tab">Filter Produk Per Kategori</a>
								</li>
							</ul>
						</div>
					</div>
					...
					<div class="tab-content">
						...
						<div class="tab-pane" id="grup">
							...
							<div class="input-group input-group-sm mb-3">
								...
								<select id="jenis_grup" name="jenis_grup[]" class="form-control custom-select2 w-80px" onchange="search_LaporanGrup()" multiple="multiple" data-allow-clear="1">
...
<form method="POST" action="<?=base_url();?>laporan/cetak_laporan_penjualan" id="target" target="_blank">
	...
	<input type="hidden" name="id_jenis" id="id_jenis" readonly />
</form>
...
<script>
	...
	$('#reportrange').on('apply.daterangepicker', function(ev, picker) {
		...
		const start_date = picker.startDate.format('DD/MM/YYYY');
		const end_date = picker.endDate.format('DD/MM/YYYY');
		$('#startdate').val(start_date);
		$('#enddate').val(end_date);
		// console.log(start_date);
		search_LaporanProduk(0,start_date,end_date);
		search_LaporanGrup(0,start_date,end_date);
		});
	...
	function search_LaporanGrup(page_num){
		...
		var start_date = $("#startdate").val();
		var end_date = $("#enddate").val();
		var jenis = $("#jenis_grup").val();
		$("#id_jenis").val(jenis);
		$("#jenis").val('grup');
		var urlnya = base_url+"laporan/pergrup/"+page_num
		$.ajax({
			type: 'POST',
			url: urlnya,
			data:{page:page_num,dari:start_date,sampai:end_date,jenis:jenis},
			beforeSend: function(){
				$('body').loading();
			},
			success: function(html){
