<div class="table-responsive table-responsive-sm">
	<table class="table table-sm table-striped table-hover">
		...
		<tbody>
			<?php if(!empty($posts)){ 
				...
				<tr>
					...
					<td>
						...
						?>
						<a data-toggle="tooltip" data-original-title="Kirim Ke <?=$nama_popup.' : '.phone_number($row["no_hp"]); ?>" data-placement="left" class="text-success kirim_wa" data-id="<?=$id_invoice;?>" data-nomor="<?=($row["no_hp"]);?>" data-trx="<?=($row["id_transaksi"]);?>"  data-tgl="<?=$row["tgl_trx"];?>" href="javascript:void(0)" ><i class="fa fa-whatsapp"></i> &nbsp;<?=$nama; ?></a>
						<?php }else{ ?>
			...
			<?php }else{ ?>
			<tr><td colspan="9">BELUM ADA PENJUALAN <?=$tanggal;?></td></tr>
			<?php } ?>
		</tbody>
