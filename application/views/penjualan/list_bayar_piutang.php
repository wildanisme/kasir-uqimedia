<?php
                ...
                <?php 
                    ...
                    foreach($bayar->result_array() AS $val){
                        ...
                        if ($this->session->level=='admin' OR $this->session->level=='owner'){
                            $hapus = '<button type="button" data-jml="'.$val['jml_bayar'].'" data-kunci="1"  data-idin="'.$val['id_invoice'].'" data-bayar="'.$val['id_bayar'].'" data-id="'.$val['id'].'"  class="btn btn-danger btn-xs delbayar_piutang"><i class="fa fa-trash"></i></button>';
                            }else{
                            $hapus = '<button type="button" data-jml="'.$val['jml_bayar'].'" data-kunci="'.$val['kunci'].'"  data-bayar="'.$val['id_bayar'].'" data-idin="'.$val['id_invoice'].'" data-id="'.$val['id'].'"  class="btn btn-danger btn-xs delbayar_piutang"><i class="fa fa-trash"></i></button>';
                        }
                    ?>
                    <tr>
                        <th scope="row"><?=$no;?>.</th>
    ...
    <input type="hidden" value="<?=$total;?>" id="jml_bayar" name="jml_bayar" >
    <input type="hidden" value="<?=$no;?>" id="no_bayar" name="no_bayar" >
    <?php 
        }else{
        echo '<input type="hidden" value="0" id="jml_bayar" name="jml_bayar" >';
