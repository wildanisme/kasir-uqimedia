<script>
    function mediamodal() {
        ...
        $("#getmedia").load('<?= base_url("whatsapp/get_media") ?>');
	}
	...
	function getmedia(media) {
        ...
        document.getElementById('input_media').value = media;
		
	$("#live_preview").html(`<a href="javascript:void(0)"><img class="card-img" style="width: 20%;" src="` + mm + `" onclick="mediamodal()"/></a>`);
    }
	
	function uploadmedia() {
	$("#getmedia").html('<div class="alert alert-primary alert-style-light text-center" role="alert"><div class="spinner-grow" style="height: 20px; width: 20px;" role="status"><span class="visually-hidden"></span></div>Uploading Media..</div>');
	...
	if (msg != 'berhasil') {
	sweet('Peringatan!!!',msg,'warning','warning');
	}
	$("#getmedia").load('<?= base_url("whatsapp/get_media") ?>');
	},
	error: function() {
...
<div class="modal modal-fullscreen-xl" id="modalmedia" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
