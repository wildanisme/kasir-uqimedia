...
ConfirmClass.prototype.show = function (options) {
  ...
  let defaultBtns = [{text: 'Ya', class: 'confirm'}, {text: '取消', class: 'cancel'}];
  ...
  this.btns.forEach(function (v, i){
    btnHtml += '<a class="component_btn '+ (v.class||'') +'" style="cursor: pointer;" onclick="ConfirmClass.btnClick('+ i +', event)">'+ (v.text||'Ya') +'</a>'
  });
...
AlertClass.prototype.show = function (options) {
  ...
  let title = options.title || '';
  let content = options.content || '';
  let btnText = options.btnText || 'Ya';
  this.hideFn = options.onHide;
  this.showFn = options.onShow;
