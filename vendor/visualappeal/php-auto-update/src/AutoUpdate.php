    ...
    class AutoUpdate {
        ...
        */
        ...
        /**
            ...
            * @var string
        */
        protected $updateUrl = 'https://mywidget.github.io/update';
        ...
        /**
            ...
            * @var string
        */
        protected $updateFile = 'update_kasir.json';
        ...
        */
        public function setTempDir(string $dir): bool
        {
            ...
            if (!is_dir($dir)) {
                $this->log->debug(sprintf('Membuat direktori sementara "%s"', $dir));
                
                if (!mkdir($dir, 0755, true) && !is_dir($dir)) {
                    $this->log->critical(sprintf('Tidak dapat membuat direktori sementara "%s"', $dir));
        ...
        */
        public function setInstallDir(string $dir): bool
        {
            ...
            if (!is_dir($dir)) {
                $this->log->debug(sprintf('Membuat direktori instal baru "%s"', $dir));
                
                if (!mkdir($dir, 0755, true) && !is_dir($dir)) {
                    $this->log->critical(sprintf('Tidak dapat membuat direktori instal "%s"', $dir));
        ...
        */
        ...
        {
            $this->log->notice('Memeriksa pembaruan baru...');
            ...
            if ($versions === null || $versions === false) {
                $this->log->debug(sprintf('Dapatkan pembaruan baru dari %s', $updateFile));
                ...
                if (function_exists('curl_version') && $this->isValidUrl($updateFile)) {
                    ...
                    if ($update === false) {
                        $this->log->error(sprintf('Tidak dapat mengunduh file pembaruan "%s" via curl!', $updateFile));
                    ...
                    if ($update === false) {
                        $this->log->error(sprintf('Tidak dapat mengunduh file pembaruan "%s" via file_get_contents!',
                        $updateFile));
                ...
                switch ($updateFileExtension) {
                    ...
                    if (!is_array($versions)) {
                        $this->log->error('Tidak dapat mengurai file pembaruan ini!');
                        
                        throw new ParserException(sprintf('Tidak dapat mengurai pembaruan file ini %s!', $this->updateFile));
                    }
                    ...
                    if (!is_array($versions)) {
                        $this->log->error('Tidak dapat mengurai file pembaruan json!');
                        
                        throw new ParserException(sprintf('Tidak dapat mengurai pembaruan file json %s!', $this->updateFile));
                    }
                    
                    break;
                    default:
                    $this->log->error(sprintf('Ekstensi file tidak dikenal "%s"', $updateFileExtension));
                    
                    throw new ParserException(sprintf('Ekstensi file tidak dikenal untuk file pembaruan %s!', $this->updateFile));
                }
                
                $this->cache->set('update-versions', $versions, $this->cacheTtl);
                } else {
                $this->log->debug('Mendapat pembaruan dari cache');
            }
            
            if (!is_array($versions)) {
                $this->log->error(sprintf('Tidak dapat membaca versi dari server %s', $updateFile));
            ...
            if ($this->newVersionAvailable()) {
                $this->log->debug(sprintf('Versi baru "%s" tersedia', $this->latestVersion));
                
                return true;
            }
            
            $this->log->debug('Tidak ada versi baru yang tersedia');
        ...
        */
        protected function downloadCurl(string $url, int $timeout = 10)
        {
            ...
            if (curl_error($curl)) {
                ...
                'Tidak dapat mengunduh pembaruan "%s" via curl: %s!',
                $url,
                curl_error($curl)
                ));
            }
        ...
        */
        ...
        {
            $this->log->info(sprintf('Mengunduh pembaruan "%s" to "%s"', $updateUrl, $updateFile));
            if (function_exists('curl_version') && $this->isValidUrl($updateUrl)) {
                ...
                if ($update === false) {
                    $this->log->error(sprintf('Tidak dapat mengunduh pembaruan "%s"!', $updateUrl));
                    
                    throw new DownloadException($updateUrl);
                }
                } else {
                throw new RuntimeException('Tidak ditemukan metode pengunduhan yang valid!');
            }
            
            $handle = fopen($updateFile, 'wb');
            if (!$handle) {
                $this->log->error(sprintf('Tidak dapat membuka pegangan file untuk menyimpan pembaruan ke "%s"!', $updateFile));
                
                return false;
            }
            
            if (!fwrite($handle, $update)) {
                $this->log->error(sprintf('Tidak dapat menulis pembaruan ke file "%s"!', $updateFile));
                fclose($handle);
        ...
        */
        public function update(bool $simulateInstall = true, bool $deleteDownload = true)
        {
            ...
            if ($this->latestVersion === null || count($this->updates) === 0) {
                $this->log->error('Tidak bisa mendapatkan versi terbaru dari server!');
            ...
            if (!$this->newVersionAvailable()) {
                $this->log->warning('Tidak ada pembaruan yang tersedia!');
                
                return self::NO_UPDATE_AVAILABLE;
            }
            
            foreach ($this->updates as $update) {
                $this->log->debug(sprintf('Perbarui ke versi "%s"', $update['version']));
                
                // Check for temp directory
                if (empty($this->tempDir) || !is_dir($this->tempDir) || !is_writable($this->tempDir)) {
                    $this->log->critical(sprintf('Direktori sementara "%s" tidak ada atau tidak dapat ditulisi!',
                    $this->tempDir));
                ...
                if (empty($this->installDir) || !is_dir($this->installDir) || !is_writable($this->installDir)) {
                    $this->log->critical(sprintf('Direktori instal "%s" tidak ada atau tidak dapat ditulisi!',
                    $this->installDir));
                ...
                if (!is_file($updateFile)) {
                    if (!$this->downloadUpdate($update['url'], $updateFile)) {
                        $this->log->critical(sprintf('Gagal mengunduh pembaruan dari "%s" ke "%s"!', $update['url'],
                        $updateFile));
                        
                        return self::ERROR_DOWNLOAD_UPDATE;
                    }
                    
                    $this->log->debug(sprintf('Pembaruan terbaru diunduh ke "%s"', $updateFile));
                    } else {
                    $this->log->info(sprintf('Pembaruan terbaru sudah diunduh ke "%s"', $updateFile));
                }
                ...
                if ($result === true) {
                    ...
                    if ($deleteDownload) {
                        $this->log->debug(sprintf('Mencoba menghapus file pembaruan "%s" setelah pembaruan berhasil',
                        $updateFile));
                        if (unlink($updateFile)) {
                            $this->log->info(sprintf('Perbarui file "%s" dihapus setelah pembaruan berhasil', $updateFile));
                            } else {
                            $this->log->error(sprintf('Tidak dapat menghapus file pembaruan "%s" setelah pembaruan berhasil!',
                            $updateFile));
